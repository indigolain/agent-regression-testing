# agent-regression-testing

## 0.1.15

### Patch Changes

- [#35](https://github.com/indigolain/agent-regression-testing/pull/35) [`81bca87`](https://github.com/indigolain/agent-regression-testing/commit/81bca87e67d2ed5494fff80aa102c75fb721da6c) Thanks [@indigolain](https://github.com/indigolain)! - Fix changeset publish command by removing unsupported --access flag

## 0.1.14

### Patch Changes

- [#28](https://github.com/indigolain/agent-regression-testing/pull/28) [`1e7e316`](https://github.com/indigolain/agent-regression-testing/commit/1e7e316454b3b5e0986d0ae2b94e144a536b8424) Thanks [@indigolain](https://github.com/indigolain)! - Fix vite security vulnerabilities (CVE-2025-v2wj, CVE-2025-p9ff, CVE-2025-4w7w) via pnpm overrides and bump dev dependencies (vitest 4.1.2 → 4.1.4, @types/node 25.5.2 → 25.6.0)

## 0.1.13

### Patch Changes

- [#23](https://github.com/indigolain/agent-regression-testing/pull/23) [`1a56780`](https://github.com/indigolain/agent-regression-testing/commit/1a56780cc9916fe15f4565f1450f0e4781d0a0b2) Thanks [@indigolain](https://github.com/indigolain)! - Add `ai` keyword for npm discoverability

## 0.1.12

### Patch Changes

- [#20](https://github.com/indigolain/agent-regression-testing/pull/20) [`da10d45`](https://github.com/indigolain/agent-regression-testing/commit/da10d45a2a1ae2fbb73cd02bccc45879c9a1114b) Thanks [@indigolain](https://github.com/indigolain)! - Add `engines` field to specify minimum Node.js version (>=22)

## 0.1.11

### Patch Changes

- [#18](https://github.com/indigolain/agent-regression-testing/pull/18) [`957d259`](https://github.com/indigolain/agent-regression-testing/commit/957d259d51b4cdd08a363fda4380548babf68888) Thanks [@indigolain](https://github.com/indigolain)! - Add `typecheck` script for running type checking without emitting files
